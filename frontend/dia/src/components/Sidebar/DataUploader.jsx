...
export default function DataUploader({ onDataLoaded, onDataRemoved, loadedDataSheets = [] }) {
  ...
  const handleFile = async (selectedFile) => {
    ...
    try {
      ...
      await localforage.setItem(RAW_DATA_KEY, parsedSheets);

      setFile(selectedFile);
      setSheets(parsedSheets);
      setExpandedSheet(parsedSheets[0].sheetName);
      onDataLoaded?.({ name: selectedFile.name, sheets: parsedSheets });

      // Only sheet names and column headers are synced; row data never leaves the device.
      if (!DEMO_MODE) {
        try {
          await apiRequest('/api/schema', { method: 'POST', body: toSchemaPayload(parsedSheets) });
        } catch (syncErr) {
          setWarning(`Data loaded locally, but column names could not be synced: ${syncErr.message}`);
        }
      }
    } catch (err) {
      console.error(err);
  ...
  const handleRemove = async () => {
    if (!window.confirm('Remove the loaded data from this browser? Configured charts keep their current values until you load a new file.')) return;
    setIsRemoving(true);
    setError(null);
    setWarning(null);
    try {
      ...
      if (!DEMO_MODE) {
        try {
          ...
          setWarning(`Local data removed, but stored column names could not be deleted: ${syncErr.message}`);
        }
      }
      setFile(null);
      setSheets([]);
  ...
  const activeSheets = sheets.length > 0 ? sheets : (loadedDataSheets || []);
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mx-2 mt-2">
          <div className="flex items-start">
            ...
            <div className="min-w-0 flex-1">
              <h4 className="text-sm font-bold text-slate-800 truncate">{file ? file.name : 'Saved workspace data'}</h4>
              <p className="text-xs text-emerald-600 font-semibold mt-1">{activeSheets.length} sheet{activeSheets.length === 1 ? '' : 's'} · stored only in this browser</p>
            </div>
          </div>
          <div className="flex space-x-2 mt-3">
            <button
              ...
              title="Load a new file. Configured charts are recalculated from the new data."
            >
              <i className="fa-solid fa-rotate mr-1.5"></i>Replace file
            </button>
            <button
              ...
              title="Delete the loaded rows from this browser and the stored column names from the server."
            >
              {isRemoving ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <><i className="fa-solid fa-trash mr-1.5"></i>Remove data</>}
            </button>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto space-y-3 px-2 pb-2">
          {activeSheets.map((sheet, sheetIdx) => (
            <div key={sheetIdx} className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col shadow-sm">
              <div
      ...
      >
        {isUploading ? (
          ...
          <>
            ...
            <p className="text-sm text-[#64748b] mb-6">Drag & drop your Excel or CSV file here</p>
